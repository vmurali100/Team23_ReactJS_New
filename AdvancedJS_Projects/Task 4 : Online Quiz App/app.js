...
const createTimer = (duration, onTick, onEnd) => {
  ...
  if (window.Worker) {
    const scoreWorker = new Worker(URL.createObjectURL(new Blob([`
  ...
  const renderQuestion = (questions) => {
    if (currentQuestionIndex >= questions.length) {
      document.getElementById("quiz-container").innerHTML = "<div class='text-center'>Quiz Completed!</div>";
  ...
  const calculateScore = (answers, correctAnswers) => {
    if (window.Worker) {
      ...
      scoreWorker.onmessage = (e) => {
        document.getElementById("quiz-container").innerHTML = `<div class='text-center font-bold text-lg'>Your Score: ${e.data}/${correctAnswers.length}</div>`;
      };
    }
  };
